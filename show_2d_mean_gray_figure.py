...
def show_figure_for_src_img(src_img, save_name):
        col_gary_value = np.sum(src_img, axis=0) / src_img.shape[0]
        col_gary_value = col_gary_value.tolist()
        row_gray_value = np.sum(src_img, axis=1) / src_img.shape[1]
        row_gray_value = row_gray_value.tolist()
        x_col = np.arange(1, src_img.shape[1] + 1).tolist()
        plt.subplot(121)
        ...
        plt.subplot(122)
        x_row = np.arange(1, src_img.shape[0] + 1).tolist()
...
def show_figure_for_src_and_dst_img(src_img, dst_img, save_name):
        dst_col_gary_value = np.sum(dst_img, axis=0) / dst_img.shape[0]
        dst_col_gary_value = dst_col_gary_value.tolist()
        dst_row_gray_value = np.sum(dst_img, axis=1) / dst_img.shape[1]
        dst_row_gray_value = dst_row_gray_value.tolist()
        dst_col = np.arange(1, dst_img.shape[1] + 1).tolist()
        ...
        dst_row = np.arange(1, dst_img.shape[0] + 1).tolist()
